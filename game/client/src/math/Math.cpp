...
bool math::rayVsRect(
	...
	common::Vector2Df invMovement = movement.inverse();

	// calc contactTime2D and farContactTime2D
	common::Vector2Df contactTime2D = (target.pos - origin) * invMovement;
	common::Vector2Df farContactTime2D = (target.pos + target.size - origin) * invMovement;
	...
	contactPoint = origin + movement * contactTime;

	// get contact normal
	if (contactTime2D.x < contactTime2D.y) { // cross vertical line before horizontal -> vertical collision
		contactNormal = { 0.0f, movement.y < 0.0f ? 1.0f : -1.0f };
	}
	else if (contactTime2D.x > contactTime2D.y) { // cross horizontal line before vertical -> horizontal collision
		contactNormal = { movement.x < 0.0f ? 1.0f : -1.0f, 0.0f };
	}
	else if (movement.x != 0 && movement.y != 0) { // contact point and normal is at a diagonal
		float cx = movement.x < 0 ? 1.0f : -1.0f * static_cast<float>(M_SQRT2);
		float cy = movement.y < 0 ? 1.0f : -1.0f * static_cast<float>(M_SQRT2);
		contactNormal = { cx, cy };
	}
...
common::Vector2Df math::resolveSweptRectVsRect(
	...
	if (hasCollision) {
		...
		common::Vector2Df speed = { std::abs(movement.x), std::abs(movement.y) };
